...
#         "Abdullah Tusin",
#     ],
#     "vendor": ["Agora", "MinaBazar", "Shopnow"],
...
class Conversation_Intent_Metadata(Base):
...
def create_database_rasa(database_engine: Engine, database_name: Text):
...
class rasaDB:
    ...
    def check_conversation_id_exists(self, conversation_id: Text):
        """Check if an account for `conversation_id` already exists"""        
        return self.session.query(
            self.session.query(Conversation_Intent_Metadata.conversation_id)
            .filter(Conversation_Intent_Metadata.conversation_id == conversation_id)
            .exists()
        ).scalar()
    ...
    def get_conversation_id(self, conversation_id: Text):
        if self.check_conversation_id_exists(conversation_id):
            intent = (
                self.session.query(Conversation_Intent_Metadata).filter(Conversation_Intent_Metadata.conversation_id == conversation_id).first()
            )
        return intent

    def set_conversation_id(self, conversation_id: Text, intent: Text):
        if self.check_conversation_id(conversation_id):
            sql = text("update conversation_intent_metadata set intent='"+str(intent)+"' where conversation_id='"+conversation_id+"'")
            result = self.engine.execute(sql)
            return result
    
    def check_conversation_id(self, conversation_id: Text):        
        return self.session.query(
            self.session.query(Conversation_Intent_Metadata.conversation_id)
            .filter(Conversation_Intent_Metadata.conversation_id == conversation_id)
            .exists()
    ...
    #     Session.__table__.create(self.engine, checkfirst=True)
    ...
    # def set_session_id(self, session_id: Text, phoneNumber: Text, slots: Text):
    #     if self.check_session_id(session_id):
    #         sql = text("update session_table set slots='"+str(slots)+"' where session_id='"+session_id+"'")
    #         result = self.engine.execute(sql)
    #         return result
        
    # def get_session_id(self, session_id: Text, phoneNumber: Text, slots: Text):
    #     if not self.check_session_id(session_id):
    #         self.add_session(session_id, phoneNumber, slots)
        
    #     account = (
    #         self.session.query(Session).filter(Session.session_id == session_id).first()
    #     )
    #     return account
    
    # def get_session_slots(self, session_id: Text):
    #     print(session_id)
    #     sql = text("select slots from session_table where session_id='"+session_id+"'")
    #     print(sql)
    #     result = self.engine.execute(sql)
    #     names = [row[0] for row in result]
    #     return names
        
    # def check_session_id(self, session_id: Text):        
    #     return self.session.query(
    #         self.session.query(Session.session_id)
    #         .filter(Session.session_id == session_id)
    #         .exists()
    #     ).scalar()
    
    # def add_session(self, session_id: Text, phoneNumber: Text, slots: Text):        
    #     return self.session.add(
    #         Session(session_id=session_id, phoneNumber=phoneNumber, slots=slots)
    #     )
